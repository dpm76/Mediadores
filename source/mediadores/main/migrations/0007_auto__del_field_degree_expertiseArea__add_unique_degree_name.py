...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding M2M table for field expertiseArea on 'Mediator'
        db.create_table('main_mediator_expertiseArea', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('mediator', models.ForeignKey(orm['main.mediator'], null=False)),
            ('expertisearea', models.ForeignKey(orm['main.expertisearea'], null=False))
        ))
        db.create_unique('main_mediator_expertiseArea', ['mediator_id', 'expertisearea_id'])

        # Deleting field 'Degree.expertiseArea'
        db.delete_column('main_degree', 'expertiseArea_id')

        # Adding unique constraint on 'Degree', fields ['name']
        db.create_unique('main_degree', ['name'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'Degree', fields ['name']
        db.delete_unique('main_degree', ['name'])

        # Removing M2M table for field expertiseArea on 'Mediator'
        db.delete_table('main_mediator_expertiseArea')

        # User chose to not deal with backwards NULL issues for 'Degree.expertiseArea'
        raise RuntimeError("Cannot reverse this migration. 'Degree.expertiseArea' and its values cannot be restored.")
